...
export function createTeamRoster(seed, playerRole, customNames = {}) {
  ...
  return positions.map((position, index) => {
    const name = String(customNames[position.key] || names[index]).trim();
    return {
      ...position,
      name,
      isPlayer: position.key === playerRole,
      display: position.key === playerRole ? `You — ${position.title}` : `${position.rank} ${name} — ${position.title}`
    };
  });
}

export function teamHuddleNarrative(seed, playerRole, personalization = {}) {
  const roster = createTeamRoster(seed, playerRole, personalization.teamNames);
  const region = String(personalization.region || 'Dara Lam').trim();
  ...
  return {
    ...
    paragraphs: [
      `The briefing room begins to empty, but your team stays behind. The Annex K remains projected on the wall beside a map of ${region}, crowded with routes, population centers, and handwritten coordination notes. The mission is no longer an abstract block of text—it is becoming yours.`,
