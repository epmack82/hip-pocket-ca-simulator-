...
export function generateScenario(seed, { day = 1, duration = 3, role = 'sgt' } = {}) {
  ...
  return {
    ...
    metadata: {
      ...
      site: site[0],
      mission: mission[0],
      complications: complications.map(item => item[0]),
      primaryStakeholder: stakeholders[0],
      trainingObjective: mission[2]
    }
  };
}

function countUsed(selected, field, value) {
  return selected.filter(scenario => scenario.metadata[field] === value).length;
}

function candidatePenalty(candidate, selected) {
  if (!selected.length) return 0;
  const meta = candidate.metadata;
  const previous = selected[selected.length - 1].metadata;
  const complicationUse = meta.complications.reduce((total, complication) => total + selected.filter(item => item.metadata.complications.includes(complication)).length, 0);
  const pairUse = selected.filter(item => item.metadata.site === meta.site && item.metadata.mission === meta.mission).length;
  return (
    countUsed(selected, 'site', meta.site) * 10000 +
    countUsed(selected, 'mission', meta.mission) * 8000 +
    pairUse * 30000 +
    countUsed(selected, 'environment', meta.environment) * 450 +
    countUsed(selected, 'geography', meta.geography) * 350 +
    countUsed(selected, 'primaryStakeholder', meta.primaryStakeholder) * 250 +
    complicationUse * 300 +
    (previous.site === meta.site ? 25000 : 0) +
    (previous.mission === meta.mission ? 15000 : 0) +
    (previous.environment === meta.environment ? 750 : 0)
  );
}

function countBy(scenarios, field) {
  return scenarios.reduce((counts, scenario) => {
    const value = scenario.metadata[field];
    counts[value] = (counts[value] || 0) + 1;
    return counts;
  }, {});
}

export function summarizeMissionCoverage(scenarios) {
  return {
    sites: countBy(scenarios, 'site'),
    missions: countBy(scenarios, 'mission'),
    environments: countBy(scenarios, 'environment'),
    geographies: countBy(scenarios, 'geography'),
    primaryStakeholders: countBy(scenarios, 'primaryStakeholder'),
    complications: scenarios.reduce((counts, scenario) => {
      scenario.metadata.complications.forEach(complication => { counts[complication] = (counts[complication] || 0) + 1; });
      return counts;
    }, {})
  };
}
...
export function generateMission({ seed, days, role }) {
  ...
  for (let index = 0; index < days; index += 1) {
    ...
    const candidates = Array.from({ length: 96 }, (_, candidateIndex) =>
      generateScenario(`${daySeed}-OPTION-${candidateIndex + 1}`, { day, duration: days, role })
    );
    const chosen = candidates.reduce((best, candidate) =>
      candidatePenalty(candidate, selected) < candidatePenalty(best, selected) ? candidate : best
    );
    selected.push({
      ...chosen,
      id: daySeed.toUpperCase(),
      seed: daySeed.toUpperCase(),
      generationSeed: chosen.seed,
      lead: { ...chosen.lead, id: `${daySeed.toUpperCase()}-LEAD-1` },
      metadata: { ...chosen.metadata, coverageIndex: day }
    });
  }
  return selected;
}
