const annexQuestions = [
  ...
  {
    id: 'annex-assessment',
    prompt: 'How should measures and assessment guidance in Annex K influence field activity?',
    options: ['They help the team collect evidence tied to mission progress and supported decisions', 'They replace observation and judgment', 'They require every engagement to use identical questions'],
    answer: 0,
    review: 'Review the assessment portion of Annex K. Measures focus collection and reporting on evidence relevant to mission progress and commander decisions.'
  }
];
...
export function getKnowledgeQuestions(phase, attempt = 1, missionSeed = 'HIP-TRAINING') {
  ...
  const offset = (attempt - 1) * 3;
  const pools = { annex: annexQuestions, ascope: ascopeQuestions, pmesii: pmesiiQuestions };
  const pool = pools[phase] || ascopeQuestions;
  const selected = seededOrder(pool, `${missionSeed}|${phase.toUpperCase()}`).slice(offset, offset + 3);
  return seededOrder(selected, `${selectionSeed}|ORDER`).map(question => randomizeOptions(question, selectionSeed));
}
